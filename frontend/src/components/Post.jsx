...
const Post = ({ post }) => {
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(`instagram-clone-pi-neon.vercel.app/api/v1/post/${post._id}/${action}`, { withCredentials: true })
      if (res.data.success) {
  ...
  const commentHandler = async () => {
    try {
      const res = await axios.post(
        `instagram-clone-pi-neon.vercel.app/api/v1/post/${post._id}/comment`,
        { text },
        {
  ...
  const deletePostHandler = async () => {
    try {
      const res = await axios.delete(`instagram-clone-pi-neon.vercel.app/api/v1/post/delete/${post._id}`, { withCredentials: true })
      if (res.data.success) {
  ...
  const bookmarkHandler = async () => {
    try {
      const res = await axios.get(`instagram-clone-pi-neon.vercel.app/api/v1/post/${post?._id}/bookmark`, { withCredentials: true })
      if (res.data.success) {
        toast.success(res.data.message)
      }
    } catch (error) {
